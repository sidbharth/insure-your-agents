/**
 * Screen 7.10 — Current policy coverage (placeholder).
 * WP-4 replaces this file with the full implementation; WP-0 only registers
 * the route so navigation and shell tests work end to end.
 */
export default function Coverage() {
  return (
    <div className="mx-auto max-w-shell px-6 py-8" data-testid="screen-Coverage">
      <div className="mb-1 text-2xs font-bold uppercase tracking-widest text-faint">
        Screen 7.10 · placeholder (WP-4)
      </div>
      <h1 className="text-lg">Current policy coverage</h1>
      <p className="mt-2 max-w-xl text-sm text-muted">Six coverage cards bound to policy numbers + Scenario Explorer.</p>
    </div>
  );
}
